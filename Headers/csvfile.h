...
public:
    csvfile(const std::string filename, const std::string separator = ";",bool openmode_out_only=false)
        : fs_()
        , is_first_(true)
        ...
        , special_chars_("\"")
    {
        ...
        if(openmode_out_only==false)
        {   //--- opens file in append mode
            fs_.open(filename, std::ofstream::out | std::ofstream::app);
        }
        else
        {
